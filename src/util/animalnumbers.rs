...
pub fn to_animal_names(mut number: i64) -> String {
    ...
    loop {
        let digit = number / ANIMAL_NAMES.len().pow(power) as i64;
        if !(result.is_empty() && digit == 0) {
            result.push(ANIMAL_NAMES[digit as usize]);
        }
        number -= digit * ANIMAL_NAMES.len().pow(power) as i64;
        if power > 0 {
            power -= 1;
...
pub fn to_u64(animal_names: &str) -> Result<i64, &str> {
    ...
    for i in 0..animals.len() {
        ...
        match animal_index {
            ...
            Some(_) => {
                result += (animal_index.unwrap() * ANIMAL_NAMES.len().pow(pow as u32)) as i64
            }
        }
    }
